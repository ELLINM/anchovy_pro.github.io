...
public class SheriffService implements ServiceInterface {
	...
	public boolean insertPerson(Person p) {
		...
		/*searchPersonBySsn(p.getSsn())값이 null이라면 p가 Sheriff or Criminal에 들어있는지 비교하고 
				각각 search method에서 값을 리턴받아 null값인지 확인한다. 이러한 연산에서는 크게 or연산으로 진행하고
				vo안에 있는지 확인하고 null값을 확인하기 위해서는 and연산으로 진행한다.*/
				pList.add(p);
				return true;
			}
		}
